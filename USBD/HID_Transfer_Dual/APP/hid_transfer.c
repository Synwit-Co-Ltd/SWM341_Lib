	...
	else
    {
        ...
        else if(epif & (1 << (EP_INT_OUT_NUM + 16)))	// Interrupt OUT
        {
			...
			if(USBD_RxSuccess())
			{
				...
				HID_GetOutReport(HID_INTF, Buffer, size);
			}
        }
		else if(epif & (1 << EP_INT_IN_NUM2))			// Interrupt IN
        {
			if(USBD_TxSuccess(EP_INT_IN_NUM2))
			{
				HID_SetInReport(HID_INTF2);
			}
			USBD_TxIntClr(EP_INT_IN_NUM2);
        }
        else if(epif & (1 << (EP_INT_OUT_NUM2 + 16)))	// Interrupt OUT
        {
			USBD_RxIntClr();
			if(USBD_RxSuccess())
			{
				uint8_t size = USBD_RxRead(Buffer, sizeof(Buffer));
				HID_GetOutReport(HID_INTF2, Buffer, size);
			}
        }
    }
}
...
int32_t HID_CmdReadPage(uint8_t intf, CMD_T *pCmd)
{
	...
	HID_SetInReport(intf);

    return 0;
}


int32_t ProcessCommand(uint8_t intf, uint8_t *buf, uint32_t len)
{
    uint32_t i, sum = 0;
...
void HID_GetOutReport(uint8_t intf, uint8_t *buf, uint32_t size)
{
    ...
    {
        ProcessCommand(intf, buf, size);
    }
	
	USBD_RxReady(intf == HID_INTF ? EP_INT_OUT_NUM : EP_INT_OUT_NUM2);
}
...
void HID_SetInReport(uint8_t intf)
{
    if(gCmd.u8Cmd == HID_CMD_READ)		// data phase of read command
    {
        if(IndexInPageBuf)
        {
            ...
            USBD_TxWrite(intf == HID_INTF ? EP_INT_IN_NUM : EP_INT_IN_NUM2, (uint8_t *)&PageBuf[PAGE_SIZE - IndexInPageBuf], EP_INT_IN_PKSZ);
